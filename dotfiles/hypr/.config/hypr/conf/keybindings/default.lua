...
-- Applications
hl.bind(mainMod .. " + RETURN", hl.dsp.exec_cmd("~/.config/xcloud/settings/terminal.sh"), { description = "Open the terminal" })
hl.bind(mainMod .. " + B", hl.dsp.exec_cmd("~/.config/xcloud/settings/browser.sh"), { description = "Open the browser" })
hl.bind(mainMod .. " + E", hl.dsp.exec_cmd("~/.config/xcloud/settings/filemanager"), { description = "Open the filemanager" })
hl.bind(mainMod .. " + CTRL + E", hl.dsp.exec_cmd("~/.config/xcloud/settings/emojipicker.sh"), { description = "Open the emoji picker" })
hl.bind(mainMod .. " + C", hl.dsp.exec_cmd("~/.config/xcloud/settings/calculator.sh"), { description = "Open the calculator" })
...
hl.bind(mainMod .. " + CTRL + W", hl.dsp.exec_cmd("~/.config/xcloud/scripts/xcloud-wallpaper-app"), { description = "Open wallpaper selector" })
hl.bind(mainMod .. " + ALT + W", hl.dsp.exec_cmd("~/.config/xcloud/scripts/xcloud-wallpaper-automation"), { description = "Start random wallpaper script" })
...
hl.bind(mainMod .. " + SHIFT + B", hl.dsp.exec_cmd("~/.config/xcloud/scripts/xcloud-reload-statusbar"), { description = "Reload Status Bar" })
hl.bind(mainMod .. " + CTRL + B", hl.dsp.exec_cmd("~/.config/xcloud/scripts/xcloud-toggle-statusbar"), { description = "Toggle Status Bar" })
hl.bind(mainMod .. " + ALT + B", hl.dsp.exec_cmd("~/.config/xcloud/scripts/xcloud-toggle-statusbar-autohide"), { description = "Toggle Status Bar Autohide" })
hl.bind(mainMod .. " + SHIFT + D", hl.dsp.exec_cmd("~/.config/xcloud/scripts/xcloud-reload-dock"), { description = "Reload Dock" })
hl.bind(mainMod .. " + CTRL + D", hl.dsp.exec_cmd("~/.config/xcloud/scripts/xcloud-toggle-dock"), { description = "Toggle Dock" })
...
hl.bind(mainMod .. " + CTRL + C", hl.dsp.exec_cmd("qs ipc call calendar toggle"), { description = "Open xCloud Calendar widget" })
hl.bind(mainMod .. " + ALT + SPACE", hl.dsp.exec_cmd("qs ipc call statusbar focus"), { description = "Expand statusbar and focus it for keyboard navigation" })
hl.bind(mainMod .. " + ALT + G", hl.dsp.exec_cmd("~/.config/hypr/scripts/gamemode.sh"), { description = "Toggle game mode" })
hl.bind(mainMod .. " + CTRL + L", hl.dsp.exec_cmd("~/.config/xcloud/scripts/xcloud-power -l"), { description = "Lock Screen" })
hl.bind(mainMod .. " + SHIFT + H", hl.dsp.exec_cmd("~/.config/xcloud/scripts/xcloud-toggle-hyprsunset"), { description = "Toggle Hyprsunset" })
...
hl.bind("CTRL + ALT + T", hl.dsp.exec_cmd("~/.config/xcloud/themes/themes.sh"), { description = "Open Select Window Menu" })
...
-- Laptop multimedia keys for volume and LCD brightness. Each chains a
-- `qs ipc call osd ...` after the real wpctl/brightnessctl command, so the
-- native OSD (OsdApp/OsdWindow.qml) shows the resulting value -- it never
-- changes volume/brightness itself, only reads it back.
hl.bind("XF86AudioRaiseVolume", hl.dsp.exec_cmd("wpctl set-volume -l 1 @DEFAULT_AUDIO_SINK@ 5%+ && qs ipc call osd showVolume"), { locked = true, repeating = true, description = "Raise volume" })
hl.bind("XF86AudioLowerVolume", hl.dsp.exec_cmd("wpctl set-volume @DEFAULT_AUDIO_SINK@ 5%- && qs ipc call osd showVolume"),      { locked = true, repeating = true, description = "Lower volume" })
hl.bind("XF86AudioMute",        hl.dsp.exec_cmd("wpctl set-mute @DEFAULT_AUDIO_SINK@ toggle && qs ipc call osd showVolume"),     { locked = true, repeating = true, description = "Mute audio" })
hl.bind("XF86AudioMicMute",     hl.dsp.exec_cmd("wpctl set-mute @DEFAULT_AUDIO_SOURCE@ toggle && qs ipc call osd showMic"),      { locked = true, repeating = true, description = "Mute microphone" })
hl.bind("XF86MonBrightnessUp",  hl.dsp.exec_cmd("brightnessctl -e4 -n2 set 5%+ && qs ipc call osd showBrightness"),              { locked = true, repeating = true, description = "Increase brightness" })
hl.bind("XF86MonBrightnessDown",hl.dsp.exec_cmd("brightnessctl -e4 -n2 set 5%- && qs ipc call osd showBrightness"),              { locked = true, repeating = true, description = "Decrease brightness" })
