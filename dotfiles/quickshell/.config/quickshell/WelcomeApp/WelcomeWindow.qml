                ...
                XCloudMenuItem { 
                    ...
                    onClicked: { 
                        if (root.isHyprlandSettingsInstalled) {
                            ...
                            Quickshell.execDetached(["kitty", "--class", "dotfiles-floating", "-e", Quickshell.env("HOME") + "/.config/xcloud/scripts/xcloud-install-hyprmod"])
                        }
                    }
                }
                background: Rectangle {
        ...
        Rectangle {
            ...
            ColumnLayout {
                ...
                ColumnLayout {
                    ...
                    Text {
                        Layout.alignment: Qt.AlignHCenter
                        text: "Welcome to xCloud Dotfiles"
                        font.family: Theme.fontFamily
                        font.pixelSize: 28
                        font.bold: true
                ...
                RowLayout {
                    ...
                    Switch {
                        ...
                        Process {
                            command: ["bash", "-c", "test -f ~/.cache/xcloud-welcome-autostart && echo exists || echo missing"]
                            running: root.visible
                            stdout: StdioCollector {
                                onStreamFinished: {
                        ...
                        onClicked: {
                            if (!ready) return;
                            if (checked) {
                                Quickshell.execDetached(["rm", "-f", Quickshell.env("HOME") + "/.cache/xcloud-welcome-autostart"])
                            } else {
                                Quickshell.execDetached(["touch", Quickshell.env("HOME") + "/.cache/xcloud-welcome-autostart"])
                            }
                        }
                    }
                }                
