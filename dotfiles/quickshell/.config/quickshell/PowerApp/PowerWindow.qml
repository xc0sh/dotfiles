...
PanelWindow {
    ...
    function activateSelected() {
        var commands = [
            Quickshell.env("HOME") + "/.config/xcloud/scripts/xcloud-power -l",
            Quickshell.env("HOME") + "/.config/xcloud/scripts/xcloud-power -s",
            Quickshell.env("HOME") + "/.config/xcloud/scripts/xcloud-power -e",
            Quickshell.env("HOME") + "/.config/xcloud/scripts/xcloud-power -r",
            Quickshell.env("HOME") + "/.config/xcloud/scripts/xcloud-power -p"
        ]
    ...
    Item {
        ...
        ColumnLayout {
            ...
            PowerButton {
                ...
                onClicked: { Quickshell.execDetached(["bash", "-c", Quickshell.env("HOME") + "/.config/xcloud/scripts/xcloud-power -l"]) }
            }
            PowerButton {
                iconSrc: "../shared/icons/suspend.svg"
                selected: root.selectedIndex === 1
                onClicked: { Quickshell.execDetached(["bash", "-c", Quickshell.env("HOME") + "/.config/xcloud/scripts/xcloud-power -s"]) }
            }
            PowerButton {
                iconSrc: "../shared/icons/logout.svg"
                selected: root.selectedIndex === 2
                onClicked: { Quickshell.execDetached(["bash", "-c", Quickshell.env("HOME") + "/.config/xcloud/scripts/xcloud-power -e"]) }
            }
            PowerButton {
                iconSrc: "../shared/icons/reboot.svg"
                selected: root.selectedIndex === 3
                onClicked: { Quickshell.execDetached(["bash", "-c", Quickshell.env("HOME") + "/.config/xcloud/scripts/xcloud-power -r"]) }
            }
            PowerButton {
                iconSrc: "../shared/icons/power.svg"
                selected: root.selectedIndex === 4
                onClicked: { Quickshell.execDetached(["bash", "-c", Quickshell.env("HOME") + "/.config/xcloud/scripts/xcloud-power -p"]) }
            }
        }
    }
}
