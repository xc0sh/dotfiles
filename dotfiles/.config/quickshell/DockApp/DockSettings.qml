...
Singleton {
    ...
    //
    //   1. ~/.config/xcloud-dock/dock.json     — the user override. While this
    //      file exists it is the master: the values it defines win, and the
    //      pin/unpin actions write their changes back into it.
    //   2. ~/.config/xcloud/settings/dock.json — the shipped fallback. It carries
    ...
    FileView {
        id: overrideFile
        path: Quickshell.env("HOME") + "/.config/xcloud-dock/dock.json"
        blockLoading: true
        printErrors: false
    ...
    FileView {
        id: settingsFile
        path: Quickshell.env("HOME") + "/.config/xcloud/settings/dock.json"
        blockLoading: true
