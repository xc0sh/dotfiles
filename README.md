...
- nvim's debugger (`nvim-dap`) is scoped to Python (`debugpy`) and Lua (`osv`, no external binary) only. Rust (`codelldb`) and Bash (`bashdb`) adapters would need AUR packages too — unlike `hadolint-bin` above, these weren't added since this repo doesn't otherwise lean heavily on Rust/Bash debugging; revisit if that changes.
