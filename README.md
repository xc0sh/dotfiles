...
- `.config/xcloud/settings/statusbar` supports `waybar`/`quickshell` only (it picks which bar the reload/toggle keybinds act on, and which one `hypr/conf/autostart.lua` actually starts at session launch). Default is `quickshell` (Quickshell-native statusbar + dock, both shipped `enabled: true`); switch to `waybar` via the sidebar's "Status Bar Engine" toggle or by editing the settings file directly.
...
- nvim's Dockerfile linter (`nvim-lint`'s `hadolint`) needs `hadolint-bin` from the AUR — it isn't in the official repos, and `xcloud-dotfiles-installer`'s `check_dependencies` only ever calls plain `pacman -S`, with no AUR-helper bootstrapping step to hook an AUR-only package into. Install it by hand (`paru -S hadolint-bin` or equivalent) until the installer gains real AUR support; the linter just won't fire on Dockerfiles until then.
